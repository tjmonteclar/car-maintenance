...
const ViewRecords: React.FC = () => {
  ...
  const plateNumbers = Array.from(
    new Set(records.map((record) => record.carPlate))
  ).sort();
  ...
  const filteredRecords = records.filter((record) => {
    ...
    const matchesCost =
      ...
      (costFilter === "medium" &&
        record.totalCost >= 100 &&
        record.totalCost <= 500) ||
      (costFilter === "high" && record.totalCost > 500);

    const matchesPlate =
      plateFilter === "all" || record.carPlate === plateFilter;
  ...
  return (
    <Layout pageTitle="View Records">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Header */}
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-900 font-['Tahoma']">
            Maintenance Records
          </h1>
          <p className="text-gray-600 mt-2">
            View and manage all vehicle maintenance history
          </p>
        </div>
